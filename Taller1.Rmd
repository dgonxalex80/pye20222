...
sum(tiempo)
sum(tiempo^2)
sum(tiempo-mean(tiempo)^1)
sum(tiempo-mean(tiempo)^2)
sum(tiempo-mean(tiempo)^3)
sum(tiempo-mean(tiempo)^4)
```
...
b. No exceder las 10 horas para la atención. Solo está permitido un 5 % de la muestra analizada.

c. Tener un 80 % de los tiempos por debajo de las 8 horas (que equivalen a un dı́a laboral).
